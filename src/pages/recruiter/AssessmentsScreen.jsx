      ...
      {steps.map((step, i) => (
        <div key={i} className={`flex items-start gap-3 p-3 rounded-xl transition-all duration-200 ${i === current ? 'bg-[#111113]' : ''}`}>
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0 mt-0.5 transition-all duration-300 ${
            i < current ? 'bg-[#06B6D4] text-[#0C0C0E]' : i === current ? 'border-2 border-[#06B6D4] text-[#06B6D4]' : 'border border-[#27272A] text-[#52525B]'
          }`}>
  ...
  return (
    <div>
      <label className="flex items-center gap-1.5 mb-2">
        ...
        {optional ? <span className="text-[10px] text-[#3F3F46] normal-case">(optional)</span> : <span className="text-[#F43F5E] text-xs">*</span>}
      </label>
      {children}
    </div>
  );
        ...
        <div className="flex items-center gap-1 bg-[#111113] border border-[#27272A] rounded-lg p-0.5">
          {statusTabs.map(({ key, label, count }) => (
            <button key={key} onClick={() => setFilter(key)}
              className={`flex items-center gap-1 px-2.5 py-1 text-[11px] font-semibold rounded-md transition-all duration-150 ${filter === key ? 'bg-[#1C1C20] text-[#E4E4E7]' : 'text-[#52525B] hover:text-[#A1A1AA]'}`}>
              {label}<span className={`text-[10px] ${filter === key ? 'text-[#A1A1AA]' : 'text-[#3F3F46]'}`}>{count}</span>
            </button>
          ))}
        </div>
        <button onClick={onClose} className="p-1.5 text-[#52525B] hover:text-[#A1A1AA] hover:bg-[#1C1C20] rounded-md transition-all">
          <X className="w-3.5 h-3.5" />
        </button>
      </div>
      {loading ? (
        <div className="flex justify-center py-8"><Loader className="w-4 h-4 text-[#06B6D4] animate-spin" /></div>
      ) : filtered.length === 0 ? (
        ...
        <div className="divide-y divide-[#1C1C20]">
          ...
          {filtered.map((c, idx) => (
            <div key={c.id} className="grid grid-cols-[minmax(0,1fr)_120px_120px_110px_80px] gap-4 items-center px-5 py-3 hover:bg-[#111113] transition-colors">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-7 h-7 rounded-full bg-[#17171A] border border-[#27272A] flex items-center justify-center text-[10px] font-bold text-[#A1A1AA] flex-shrink-0 font-display">{getInitials(c.candidate_name)}</div>
              ...
              <div className="flex justify-end">
                <button onClick={() => handleCopy(c.candidate_email, idx)} title="Copy email"
                  className="p-1.5 text-[#3F3F46] hover:text-[#06B6D4] hover:bg-[#083344] rounded-md transition-all">
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[20px] font-bold text-[#FAFAFA] font-display tracking-tight">Assessments</h1>
          <p className="text-[13px] text-[#52525B] mt-0.5">Configure and manage technical assessments.</p>
        </div>
        <button onClick={openWizard} className="flex items-center gap-2 px-4 py-2.5 bg-[#06B6D4] hover:bg-[#0891B2] text-[#0C0C0E] text-[13px] font-bold rounded-lg transition-colors active:scale-[0.97]">
      ...
      {/* Toasts */}
      {success && (
        <div className="mb-5 flex items-center gap-3 px-4 py-3 bg-[#022C22] border border-[#065F46] rounded-xl">
      ...
      <div className="flex gap-5 items-start">
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 px-3 py-2 bg-[#111113] border border-[#27272A] rounded-lg flex-1 min-w-[160px] focus-within:border-[#3F3F46]">
            ...
            <div className="flex items-center gap-0.5 bg-[#111113] border border-[#27272A] rounded-lg p-1">
              {[
                ...
                <button key={key} onClick={() => setStatusFilter(key)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold rounded-md transition-all ${statusFilter === key ? 'bg-[#1C1C20] text-[#E4E4E7]' : 'text-[#52525B] hover:text-[#A1A1AA]'}`}>
                  {label}<span className={`text-[10px] font-bold ${statusFilter === key ? 'text-[#A1A1AA]' : 'text-[#3F3F46]'}`}>{count}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {!loading && assessments.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <button onClick={openWizard} className="flex items-center gap-2 px-4 py-2.5 bg-[#06B6D4] hover:bg-[#0891B2] text-[#0C0C0E] text-[13px] font-bold rounded-lg active:scale-[0.97]">
                ...
                {filtered.map(assessment => {
                  ...
                  return (
                    <div key={assessment.id}>
                      <div
                        className={`group flex md:grid md:grid-cols-[minmax(0,1fr)_80px_160px_140px_80px_120px] gap-4 items-center px-5 py-4 cursor-pointer flex-wrap transition-colors ${isExpanded ? 'bg-[#111113]' : 'bg-[#0C0C0E] hover:bg-[#111113]'}`}
                        onClick={() => toggleExpand(assessment.id)}
                      >
                        <div className="min-w-0 w-full md:w-auto">
                          <div className="flex items-center gap-2 mb-0.5">
                            <ChevronDown className={`w-3 h-3 text-[#3F3F46] flex-shrink-0 transition-transform ${isExpanded ? 'rotate-0' : '-rotate-90'}`} />
                            <p className="text-[13px] font-semibold text-[#E4E4E7] truncate">{assessment.name}</p>
                          </div>
                          {task ? (
                            <p className="flex items-center gap-1 text-[11px] text-[#52525B] ml-5 truncate"><Code className="w-3 h-3 flex-shrink-0" />{task.title}</p>
                          ) : (
                            <p className="text-[11px] text-[#3F3F46] italic ml-5">No task configured</p>
                          )}
                        </div>
                        <div className="hidden md:flex items-center gap-1.5 text-[12px] text-[#A1A1AA]"><Clock className="w-3 h-3 text-[#52525B]" />{assessment.duration_minutes}m</div>
      ...
      {showWizard && (
        <>
          <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-40" onClick={closeWizard} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <div className="w-full max-w-[760px] bg-[#111113] border border-[#27272A] rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[88vh] pointer-events-auto" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex items-center justify-between px-7 py-5 border-b border-[#27272A] flex-shrink-0">
                ...
                <button onClick={closeWizard} className="w-8 h-8 flex items-center justify-center text-[#52525B] hover:text-[#E4E4E7] hover:bg-[#1C1C20] rounded-lg">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex flex-1 overflow-hidden min-h-0">
                <div className="w-56 flex-shrink-0 hidden sm:flex flex-col gap-5 bg-[#0C0C0E] border-r border-[#27272A] p-5">
                  ...
                  {wizardStep === 1 && assessmentForm.name && (
                    <div className="p-3 rounded-xl border border-[#27272A] bg-[#111113]">
                          ...
                          <div className="flex gap-2 mb-3 flex-wrap">
                            {DURATION_PRESETS.map(p => (
                              <button key={p} type="button" onClick={() => setAssessmentForm({ ...assessmentForm, duration_minutes: String(p) })}
                                className={`px-3 py-1.5 text-[12px] font-semibold rounded-lg border transition-all ${assessmentForm.duration_minutes === String(p) ? 'bg-[#083344] border-[#06B6D4] text-[#06B6D4]' : 'bg-transparent border-[#27272A] text-[#52525B] hover:border-[#3F3F46] hover:text-[#A1A1AA]'}`}>
                                {p}m
                              </button>
                            ))}
                          </div>
                      ...
                      <div className="space-y-5">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              <button key={value} type="button" onClick={() => { setTaskForm({ ...taskForm, source_type: value }); resetUpload(); }}
                                className={`flex items-center justify-center gap-2 py-3 text-[13px] font-semibold rounded-xl border transition-all ${taskForm.source_type === value ? 'bg-[#083344] border-[#06B6D4] text-[#06B6D4]' : 'bg-transparent border-[#27272A] text-[#52525B] hover:border-[#3F3F46] hover:text-[#A1A1AA]'}`}>
                        ...
                        {taskForm.source_type === 'local' && (
                          <div className="rounded-xl border border-[#27272A] bg-[#0C0C0E] p-4">
                            ...
                            {uploadState === 'idle' && (
                              <label className="flex flex-col items-center justify-center gap-3 py-8 border border-dashed border-[#27272A] hover:border-[#06B6D4]/40 rounded-xl cursor-pointer transition-all hover:bg-[#06B6D4]/[0.02]">
                            ...
                            {(uploadState === 'zipping' || uploadState === 'uploading') && (
                              <div className="flex flex-col items-center gap-3 py-6">
                                ...
                                {uploadState === 'uploading' && <div className="w-full h-1 bg-[#1C1C20] rounded-full overflow-hidden"><div className="h-full bg-[#06B6D4] rounded-full transition-all" style={{ width: `${uploadProgress}%` }} /></div>}
                              </div>
                            )}
                            {uploadState === 'done' && (
                              <div className="flex items-center gap-3 px-4 py-3 bg-[#022C22] border border-[#065F46] rounded-xl">
                                <CheckCircle className="w-4 h-4 text-[#10B981] flex-shrink-0" />
                                ...
                                <button onClick={resetUpload} className="p-1.5 text-[#52525B] hover:text-[#F43F5E] rounded-md flex-shrink-0"><XCircle className="w-4 h-4" /></button>
                              </div>
                            )}
                            {uploadState === 'error' && (
                              <div className="space-y-3">
                                ...
                                <button onClick={resetUpload} className="flex items-center gap-2 text-[12px] text-[#06B6D4] hover:underline"><Upload className="w-3.5 h-3.5" /> Try again</button>
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        {taskForm.source_type === 'git' && (
                          <div className="rounded-xl border border-[#27272A] bg-[#0C0C0E] p-4 space-y-4">
              ...
              <div className="flex items-center justify-between px-7 py-4 border-t border-[#27272A] bg-[#0C0C0E] flex-shrink-0">
                {wizardStep === 1 ? (
                  <button onClick={() => { setWizardStep(0); setWizardError(''); }} className="flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-[#A1A1AA] hover:text-[#E4E4E7] hover:bg-[#1C1C20] rounded-lg transition-all">← Back</button>
                ) : (
                  <button onClick={closeWizard} className="px-4 py-2 text-[13px] font-medium text-[#52525B] hover:text-[#A1A1AA] hover:bg-[#1C1C20] rounded-lg transition-all">Cancel</button>
                )}
                <button onClick={wizardStep === 0 ? handleStep1Next : handleFinalCreate} disabled={wizardLoading}
                  className="flex items-center gap-2 px-5 py-2.5 bg-[#06B6D4] hover:bg-[#0891B2] text-[#0C0C0E] text-[13px] font-bold rounded-lg transition-all active:scale-[0.97] disabled:opacity-40 disabled:cursor-not-allowed">
                  {wizardLoading ? (
